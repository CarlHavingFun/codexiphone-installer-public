...
if (-not $ManifestUrl) {
  if ($env:CODEXIPHONE_MANIFEST_URL) {
    ...
    $ManifestUrl = "https://my-agent.com.cn/codexiphone/runtime-manifest.json"
  }
}
...
try {
  ...
  Write-NetworkHint "$(if ($PlatformBaseUrl) { $PlatformBaseUrl } else { 'https://my-agent.com.cn/codex-platform' })"
  throw
}
